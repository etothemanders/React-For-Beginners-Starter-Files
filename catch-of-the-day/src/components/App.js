...
class App extends React.Component {
	...
	componentDidMount() {
		...
		// reinstate our order state from localStorage
	...
	addFish = fish => {
		// 1. copy existing piece of state
	...
	deleteFish = (key) => {
		const fishes = { ...this.state.fishes };
		fishes[key] = null; // set to null to delete the fish from firebase
		this.setState({ fishes });
	};
	...
	removeFromOrder = key => {
		const order = { ...this.state.order };
		delete order[key]; // can delete because we're not mirroring to firebase
		this.setState({ order });
	}
